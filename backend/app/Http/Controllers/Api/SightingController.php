...
class SightingController extends Controller
{
    public function index($pet)
    {
        ...
        $sightings = Sighting::where('pet_id', $petModel->id)
            ->latest()
            ->get()
    ...
    public function ownerSightings(Request $request)
    {
        $user = $request->user();
        ...
        $formatted = $pets->map(function ($pet) {
            ...
            if ($pet->lost_photo_path) {
                $photoUrl = asset('storage/' . ltrim($pet->lost_photo_path, '/'));
            ...
            return [
                ...
                'reported_lost_at' => optional($pet->reported_lost_at)?->format('Y-m-d H:i:s'),
                'photo_url' => $photoUrl,
                'sightings' => $pet->sightings->map(function ($sighting) {
                    return [
                        'id' => $sighting->id,
                        'location' => $sighting->location,
                        'lat' => $sighting->lat !== null ? (float) $sighting->lat : null,
